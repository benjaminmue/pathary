...
#[AsCommand(
    name: 'omdb:sync',
    description: 'Sync IMDb and Rotten Tomatoes ratings from OMDb API for local movies, sorted by how outdated they are (oldest first).',
    aliases: ['omdb:sync'],
    hidden: false,
)]
class OmdbSync extends Command
{
    ...
    protected function configure() : void
    {
        $this
            ...
            ->addOption(self::OPTION_NAME_NEVER_SYNC, 'never-synced', InputOption::VALUE_NONE, 'Only sync ratings for movies which were never synced before.');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        $jobId = $this->jobQueueApi->addOmdbSyncJob(JobStatus::createInProgress());

        try {
            $this->generateOutput($output, 'Syncing OMDb movie ratings (IMDb and Rotten Tomatoes)...');

            $this->omdbMovieRatingSync->syncMultipleMovieRatings($maxAgeInHours, $movieCountSyncThreshold, $movieIds, onlyNeverSynced: $onlyNeverSynced);

            $this->jobQueueApi->updateJobStatus($jobId, JobStatus::createDone());

            $this->generateOutput($output, 'Syncing OMDb movie ratings done.');
        } catch (Throwable $t) {
            $this->generateOutput($output, 'ERROR: Could not complete OMDb sync.');
            $this->logger->error('Could not complete OMDb sync.', ['exception' => $t]);
